"use client";

import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { getPolicyholderQueryHistory } from "@/lib/api";
import {
  History, Loader2, FolderOpen, ChevronDown, ChevronUp,
  MessageSquare,
} from "lucide-react";
import MarkdownResponse from "@/components/MarkdownResponse";

interface QueryItem {
  id: string;
  question: string;
  answer_preview: string;
  confidence: number | null;
  latency_ms: number | null;
  citation_count: number;
  queried_at: string;
}

export default function PolicyholderHistoryPage() {
  const router = useRouter();
  const [queries, setQueries] = useState<QueryItem[]>([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(true);
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const PAGE_SIZE = 25;

  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const data = await getPolicyholderQueryHistory(page, PAGE_SIZE);
        setQueries(data.queries || []);
        setTotal(data.total || 0);
      } catch {
        // silently fail
      } finally {
        setLoading(false);
      }
    })();
  }, [page]);

  const totalPages = Math.ceil(total / PAGE_SIZE);

  const handleContinueConversation = () => {
    router.push("/policyholder");
  };

  const handleScrollToQuery = (queryId: string) => {
    router.push(`/policyholder?scroll_to=${queryId}`);
  };

  return (
    <div className="p-4 sm:p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-semibold text-heading flex items-center gap-2">
            <History className="w-5 h-5" /> My Past Questions
          </h1>
          <p className="text-sm text-muted mt-1">
            View your previous questions and the answers you received
          </p>
        </div>
        {queries.length > 0 && (
          <button
            onClick={handleContinueConversation}
            className="flex items-center gap-1.5 px-3 py-2 text-xs sm:text-sm bg-primary-btn text-white rounded-lg hover:bg-primary-btn-hover transition font-medium"
          >
            <MessageSquare className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Continue Conversation</span>
            <span className="sm:hidden">Continue</span>
          </button>
        )}
      </div>

      <div className="mt-6">
        {loading ? (
          <div className="flex items-center justify-center py-12 text-muted">
            <Loader2 className="w-5 h-5 animate-spin mr-2" /> Loading...
          </div>
        ) : queries.length === 0 ? (
          <div className="text-center py-12 text-muted">
            <FolderOpen className="w-10 h-10 mx-auto mb-2 opacity-30" />
            <p>No questions yet</p>
            <p className="text-sm mt-1">Your past questions will appear here after you ask them.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {queries.map((q) => (
              <div
                key={q.id}
                className="bg-card rounded-xl border border-border-default overflow-hidden"
              >
                <button
                  onClick={() => setExpandedId(expandedId === q.id ? null : q.id)}
                  className="w-full text-left px-4 sm:px-5 py-3 sm:py-4 hover:bg-surface transition"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-heading break-words">{q.question}</p>
                      {expandedId !== q.id && (
                        <p className="text-xs text-muted mt-1 line-clamp-2 break-words">
                          {q.answer_preview}
                        </p>
                      )}
                    </div>
                    <div className="flex items-center gap-2 shrink-0 text-xs text-muted">
                      <span className="hidden sm:inline">
                        {new Date(q.queried_at).toLocaleDateString()}
                      </span>
                      <span className="sm:hidden">
                        {new Date(q.queried_at).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
                      </span>
                      {expandedId === q.id ? (
                        <ChevronUp className="w-4 h-4" />
                      ) : (
                        <ChevronDown className="w-4 h-4" />
                      )}
                    </div>
                  </div>
                </button>

                {expandedId === q.id && (
                  <div className="border-t border-gray-100 px-4 sm:px-5 py-3 sm:py-4">
                    <div className="text-sm text-heading break-words">
                      <MarkdownResponse content={q.answer_preview} />
                    </div>
                    <div className="flex flex-wrap items-center gap-3 mt-3 text-xs text-muted">
                      <span>{q.citation_count} citations</span>
                      {q.latency_ms != null && <span>{q.latency_ms}ms</span>}
                    </div>
                    <button
                      onClick={() => handleScrollToQuery(q.id)}
                      className="mt-3 flex items-center gap-1.5 text-xs text-accent hover:text-accent font-medium transition"
                    >
                      <MessageSquare className="w-3.5 h-3.5" />
                      View in conversation
                    </button>
                  </div>
                )}
              </div>
            ))}
          </div>
        )}

        {/* Pagination */}
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-4">
            <span className="text-sm text-muted">
              Page {page} of {totalPages}
            </span>
            <div className="flex gap-2">
              <button
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page <= 1}
                className="px-3 py-1.5 text-sm border border-border-default rounded-lg disabled:opacity-40 hover:bg-surface"
              >
                Previous
              </button>
              <button
                onClick={() => setPage((p) => Math.min(totalPages, p + 1))}
                disabled={page >= totalPages}
                className="px-3 py-1.5 text-sm border border-border-default rounded-lg disabled:opacity-40 hover:bg-surface"
              >
                Next
              </button>
            </div>
          </div>
        )}
      </div>
    </div>
  );
}
