...
export default function StaffQueryPage() {
  ...
  const [limitError, setLimitError] = useState<{ error: string; detail: string; usage?: { used: number; limit: number } } | null>(null);
  ...
  const loadConversation = useCallback(async (polNum: string) => {
    ...
    if (loadedPolicyRef.current === polNum) return;
  ...
  // Scroll to target query or to bottom
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message.includes("limit exceeded") || err.message.includes("limit")) {
        setLimitError({
          error: "Query limit exceeded",
          detail: err.message,
        });
        // Remove the user message since the query didn't go through
          ...
          <div className="max-w-3xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
            <form onSubmit={handleSubmit} className="flex gap-2 sm:gap-3">
              <input
                  ...
                  placeholder={
                    limitError
                        ? "Query limit reached — please contact your administrator"
                        : queryType === "policy"
